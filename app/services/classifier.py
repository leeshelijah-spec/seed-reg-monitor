...
CATEGORY_KEYWORDS = {
    ...
    "품질관리": ["품질", "검정", "검사", "보증", "인증", "기준", "표준", "시료", "이력", "추적", "회수"],
    "품종보호": ["품종보호", "식물신품종", "보호품종", "육성자", "품종", "품종명칭", "품종출원"],
    "인허가·영업등록": ["등록", "허가", "승인", "신고", "변경등록", "갱신", "등록취소", "영업정지", "종자업", "수입판매업"],
    "지식재산·육종권": ["육성자권", "실시권", "품종출원", "영업비밀", "기술보호", "유전자원", "지식재산"],
    "생물안전·LMO": ["유전자변형", "lmo", "생물안전", "위해성", "격리재배", "시험재배", "국가간 이동"],
    "농자재·재배환경": ["농약", "비료", "농자재", "토양", "재배환경", "친환경", "유기농", "잔류", "약해"],
...
LOW_SIGNAL_TERMS = ["지원", "협력", "원조", "정책방향", "참고", "운영규정", "활성화", "육성"]
...
class RegulationClassifier:
    ...
    def _determine_severity(self, item: dict[str, Any], text_lower: str, categories: list[str]) -> tuple[str, str]:
        ...
        if effective_in_days is not None and effective_in_days <= 30 and (penalty_hits or obligation_hits >= 2):
            ...
            or ("수입검역" in categories and operational_hits)
            or ("인허가·영업등록" in categories and obligation_hits)
            or ("품질관리" in categories and any(term in text_lower for term in ("회수", "추적", "이력")))
        ):
            severity = "중요"
    ...
    def _build_relevance_reason(
        ...
        base = f"{', '.join(categories)} 영역 키워드가 확인되어 종자 생산·수입·품질·인허가·품종보호 체계와의 연관성이 있습니다."
        if sample_match and sample_match[1] >= 0.18:
