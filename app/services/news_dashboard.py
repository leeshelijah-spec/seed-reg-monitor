    ...
    def _build_executive_summary(self, filters: NewsFilterParams) -> dict[str, list[str]]:
        ...
        with get_connection() as connection:
            ...
            urgent_rows = connection.execute(
                ...
                LIMIT 3
                """,
                [IMPORTANT, IMMEDIATE, *params, NOISE, RELEVANT],
            ).fetchall()
            feedback_summary = connection.execute(
                ...
                WHERE substr(COALESCE(published_at, collected_at), 1, 10) >= ?{where_tail}
                """,
                [RELEVANT, NOISE, seven_days_ago, *params],
            ).fetchone()

        feedback_counts = feedback_summary or {"relevant_count": 0, "noise_count": 0}

        key_trends = [
            f"최근 7일 동안 '{row['topic_category']}' 이슈가 {row['count']}건으로 가장 많이 포착되었습니다."
            for row in top_topics
        ] or ["최근 기사 수가 아직 충분하지 않아 뚜렷한 핵심 트렌드를 요약하기 어렵습니다."]

        implications = [
            f"{row['owner_department']}에서 '{row['title']}'의 {row['business_impact_level']} 영향 여부를 우선 검토하세요."
            for row in urgent_rows
        ] or ["중요도 높은 기사가 더 누적될 때까지 키워드 범위를 넓혀 모니터링을 이어가세요."]

        recommended_tasks: list[str] = []
        if (feedback_counts["relevant_count"] or 0) > 0:
            recommended_tasks.append(
                f"최근 7일 동안 관련으로 확인된 기사 {feedback_counts['relevant_count']}건은 부서별 실행계획과 주간 보고에 반영하세요."
            )
        if (feedback_counts["noise_count"] or 0) > 0:
            recommended_tasks.append(
                f"잡음으로 분류된 기사 {feedback_counts['noise_count']}건은 키워드 유지 여부와 그룹 규칙 조정 후보로 검토하세요."
            )
        recommended_tasks.extend(
            [
                "주간 경영회의 전에 중요·즉시조치 기사를 함께 검토하세요.",
                "누적된 피드백을 바탕으로 분류 및 그룹 규칙을 계속 보정하세요.",
            ]
        )
        return {
