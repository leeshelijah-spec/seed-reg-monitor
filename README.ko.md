...
- 최신 업데이트: [2026-04-02](docs/feature-updates/2026-04-02.md)
- 네이버 뉴스 수집, 분석, 대시보드, 샘플 데이터 흐름을 한 번에 추가
- 대시보드 KPI를 촘촘한 2x2 그리드로 재구성하고 뉴스 필터를 토글 패널로 정리
- 규제와 기사 검토 화면을 미검토 중심 흐름으로 재배치
- 외부 공유 시 수정 기능을 막을 수 있도록 읽기 전용 대시보드와 ngrok 공유 흐름을 추가
- 편집 모드 실행 파일, favicon 연결, 로컬 실행 기본값 정리를 추가
- 상대경로가 비어 있을 때도 유효한 `korean-law-mcp` 빌드를 자동 탐색하도록 규제 동기화를 보강
